	...
	unsigned char *dend, const char *name_src)
{
    ...
    snprintf(item_name, MAX_ITEMBUF, "%s/version(%d) flags(%u)", name_buf,
        (vf>>24) & 0xff,(vf & 0x00ffffff));
    add_sub_item();

    snprintf(item_name, MAX_ITEMBUF, "%s/sequence-number(%u)", name_buf,n);
    add_sub_item();
	...
	unsigned char *dend, const char *name_src)
{
	...
	bal = get2(s); s += 2;
    snprintf(item_name, MAX_ITEMBUF, "%s/balance(%d)", name_buf, bal);
    add_sub_item();
	...
	unsigned char *dend, const char *name_src)
{
	...
	snprintf(item_name, MAX_ITEMBUF, "%s/Entries(%u)", name_buf, en);
	add_sub_item();
	++en;
	...
	if(memcmp(box_name, "url ", 4) == 0)
  {
	...
	snprintf(ebuf, MAX_ITEMBUF, 
	 "%s/[%u] url: version(%hu) flag(%u)", name_buf, i, vers, flag);
    sub = tree->add(ebuf);
	sub->usericon(&L_documentpixmap);
  }
	else
	if(memcmp(box_name, "urn ", 4) == 0)
  {
	...
	snprintf(ebuf, MAX_ITEMBUF,
	 "%s/[%u] urn: version(%hu) flag(%u) ename(%s)", 
...
#ifdef USE_LONG_NAME
	snprintf(item_name, MAX_ITEMBUF, "%s/[%04u] entry-size(%u)",
#else
    snprintf(item_name, MAX_ITEMBUF, "%s/[%04u] size(%u)",
#endif
     name_buf,i,esize);
    add_sub_item();
  }
...
//	if(bits == 16)
  {
    ...
    snprintf(item_name, MAX_ITEMBUF, "%s/[%04u] size(%hu)",
     name_buf,i,val);
    add_sub_item();
	...
	unsigned char *dend, const char *name_src)
{
   ...
   { 
    ...
    snprintf(item_name, MAX_ITEMBUF, "%s/[%04u] offset(%u)",
	 name_buf,i,co);
    add_sub_item();
...
fprintf(stderr,"[%u]'vide' has wrong box name:(%s)\n",i,box_name);
...
LOG("stsz/stz2 missing. STOP.");
	...
	if(memcmp(box_name, "co64", 4) == 0)
   {
    ...
    (void)read_co64(s + hstep, s + box_len, dend, sup_buf);
	...
	unsigned char *dend, const char *name_src)
{
    ...
    else
  {
    ...
    snprintf(item_name, MAX_ITEMBUF, "%s/[%u] media-time(%u)", name_buf,i,t);
    add_sub_item();
  }
	irate = get2(s); s += 2;
	frate = get2(s); s += 2;
	...
	unsigned char *dend, const char *name_src)
{
	...
	snprintf(item_name, MAX_ITEMBUF, "%s/name(%s)", name_buf, (char*)s);
	add_sub_item();
...

{
	...
	for(i = 0; i < 9; ++i)
   {
	...
	snprintf(item_name, MAX_ITEMBUF, "%s/matrix[%d](%u)",name_buf, i,val);
	add_sub_item();
   }
...
fprintf(stderr,"trex\n\t"
"vf(%u,%u) trackID(%u) index(%u) dur(%u) size(%u) flags(%u)\n",
