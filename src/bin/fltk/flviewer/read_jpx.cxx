	...
	unsigned char *dend, const char *name_src)
{
	...
	for(i = 0; i < n; ++i)
   {
	...
	snprintf(item_name, MAX_ITEMBUF,"%s/frag[%u] off(%lu) len(%u) dr(%u)",
	 name_buf, i,(unsigned long)off,len,dr);
	add_sub_item();
   }
	...
	unsigned char *dend, const char *name_src)
{
	...
	typ = s[0]; ++s;
	n = s[0]; ++s;
	if(n > 0) siz = (box_end - s); else siz = 0;
...
/* Digital Signatur Box */
	...
	if(ptyp == 1)
   {
	...
	snprintf(item_name, MAX_ITEMBUF, "%s/styp(%u) ptyp(%u) off(%lu) len(%lu) data size[%lu]",
	 name_buf,styp,ptyp,(unsigned long)off,(unsigned long)len,
