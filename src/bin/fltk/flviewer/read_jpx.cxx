	...
	unsigned char *dend, const char *name_src)
{
	...
	rtyp = get4(s); s += 4;
	snprintf(item_name, MAX_ITEMBUF, "%s/rtyp(%u)",name_buf,rtyp);
	add_sub_item();
...
---------------------*/ 
	...
	for(i = 0; i < nr; ++i)
   {
	...
	snprintf(item_name, MAX_ITEMBUF, "%s/roi[%u] present(%u) typ(%u) "
	 "prio(%u) x(%u) y(%u) w(%u) h(%u)",name_buf, i,r,typ,prio,x,y,w,h);
	add_sub_item();
   }
...
rept,tick);
#endif
	snprintf(item_name, MAX_ITEMBUF, "%s/typ(%d) rept(%d) tick(%u)",
	 name_buf, typ,rept,tick);
	add_sub_item();
...
"\txc(%d) yc(%d) wc(%d) hc(%d)\n",i,x0,y0,w,h,life,persist,next_use,
 xc,yc,wc,hc);
---------------*/
	snprintf(item_name, MAX_ITEMBUF, "%s/inst[%d] x0(%u) y0(%u) w(%u) h(%u)"
	 " life(%u) persist(%u) next-use(%u)"
	 " xc(%u) yc(%u) wc(%u) hc(%u)",name_buf,i,x0,y0,w,h,life,persist,
	...
	unsigned char *dend, const char *name_src)
{
	...
	snprintf(item_name, MAX_ITEMBUF, "%s/height(%u) width(%u) loop(%hu)",
	 name_buf,h,w,loop);
	add_sub_item();
	...
	unsigned char *dend, const char *name_src)
{
	...
	if(memcmp(box_name, "inst", 4) == 0)
   {
	...
	(void)read_inst(s + hstep, s + box_len, dend, item_name);
...
/* Digital Signatur Box */
	...
	if(ptyp == 1)
   {
	...
	snprintf(item_name, MAX_ITEMBUF, 
	 "%s/styp(%u) ptyp(%u) off(%" PRIu64 ") len(%" PRIu64 ") data size[%" PRIu64 "]",
	 name_buf,styp,ptyp,off,len,(box_end-s));
	add_sub_item();
	...
	unsigned char *dend, const char *name_src)
{
	...
	if(memcmp(box_name, "gtso", 4) == 0)
   {
	...
	(void)read_gtso(s + hstep, s + box_len, dend, item_name);
	...
	unsigned char *dend, const char *name_src)
{
	...
	|| memcmp(box_name, "XML ", 4) == 0)
  {
	...
	snprintf(name, MAX_ITEMBUF, "%s/[%03d] xml ", name_buf, ++pos);
	sup = tree->add(name);

    snprintf(item_name, MAX_ITEMBUF, "%s/xml1", name);
...
LOG("flst missing. STOP.");
   ...
   }
   {
    ...
    (void)read_flst(s + hstep, s + box_len, dend, item_name);
